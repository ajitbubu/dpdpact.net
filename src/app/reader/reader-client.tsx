...
/**
 * Indent per nesting depth, matching the Gazette's clause layout. The clamps
 * keep the deepest sub-clause from eating half the line on a phone while
 * holding the Gazette's steps at reading width.
 */
const PAD = [
  "0",
  "clamp(14px,4vw,24px)",
  "clamp(26px,8vw,48px)",
  "clamp(36px,12vw,70px)",
];
...
export function ReaderClient() {
  ...
  /** Below 1020px the rail is a drawer rather than a permanent column. */
  ...
  const readingProgress =
    mode === "read" && idx >= 0
      ? Math.min(100, pageScrollProgress * 100)
      : 0;

  // Track the current section rather than dividing its scroll distance across
  // all 45 pages. The old calculation moved the bar by at most 2.2% per page,
  // which was effectively invisible even after a reader scrolled to the end.
  React.useEffect(() => {
    if (mode !== "read") return;
    function update() {
      const pane = readingPaneRef.current;
      if (!pane) return;
      const paneBottom = pane.getBoundingClientRect().bottom + window.scrollY;
      // Stop at the reader controls, before the site footer. Short sections
      // that fit without scrolling are already fully readable.
      const maxScroll = paneBottom - window.innerHeight;
      setPageScrollProgress(
        maxScroll <= 0 ? 1 : Math.min(1, window.scrollY / maxScroll),
      );
    }
    // Measured a frame later, once the new section has been laid out.
  ...
  const rail = (
    <>
      <div className="flex shrink-0 flex-col gap-[7px] border-b border-border px-[22px] pb-[18px] pt-[22px]">
      ...
      <div className="shrink-0 px-[14px] pb-[6px] pt-[14px]">
        <button
          onClick={() => {
            setModeOverride("toc");
            setQ("");
          ...
          className={cn(
            "flex w-full cursor-pointer items-center gap-[10px] rounded-[8px] border border-border px-[12px] py-[13px] text-left font-sans text-[13.5px] font-semibold leading-[1.2] text-white hover:bg-surface-raised min-[1020px]:py-[10px] min-[1020px]:text-[12.5px]",
      ...
      <nav className="flex min-h-0 flex-1 flex-col gap-[3px] overflow-y-auto px-[12px] pb-[18px] pt-[6px]">
        {CHAPTERS.map((c) => {
          const active = mode === "read" && page !== null && page.chId === c.id;
          return (
            <div key={c.id} className="flex flex-col gap-[2px]">
              <button
                ...
                className={cn(
                  "flex w-full cursor-pointer items-center gap-[10px] rounded-[8px] border-0 px-[10px] py-[12px] text-left font-sans text-[13.5px] font-semibold leading-[1.35] hover:bg-surface-raised min-[1020px]:py-[9px] min-[1020px]:text-[13px]",
                  active
                  ...
                  {c.sections.map((s) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => goTo(i)}
                        className={cn(
                          "flex w-full cursor-pointer items-center gap-[8px] rounded-[7px] border-0 px-[9px] py-[10px] text-left font-sans text-[13.5px] leading-[1.4] hover:bg-surface-raised min-[1020px]:py-[6px] min-[1020px]:text-[12.5px]",
                          on
        ...
        <button
          ...
          className={cn(
            "mt-[2px] flex w-full cursor-pointer items-center gap-[10px] rounded-[8px] border-0 px-[10px] py-[12px] text-left font-sans text-[13.5px] font-semibold leading-[1.35] hover:bg-surface-raised min-[1020px]:py-[9px] min-[1020px]:text-[13px]",
            mode === "read" && page?.kind === "sch"
              ? "bg-primary-tint text-text"
              : "bg-transparent text-text-secondary",
          )}
        >
          ...
          <span className="min-w-0 flex-1">The Schedule</span>
          <span className="shrink-0 font-mono text-[12px] font-semibold leading-none text-text-muted">
            1
          </span>
        </button>
  ...
  return (
    <>
      ...
      <div className="grid flex-1 font-sans text-text min-[1020px]:grid-cols-[314px_minmax(0,1fr)]">
        {/* ------------------------------------------------- Dark rail */}
        <aside
          className="sticky hidden min-h-0 flex-col bg-[#14140F] min-[1020px]:flex"
          style={{
            ...RAIL_TOKENS,
            top: NAV_H,
        ...
        {/* --------------------------------------- Rail as mobile drawer */}
        {drawerOpen && (
          <div className="fixed inset-0 z-[80] flex min-[1020px]:hidden">
            <button
              aria-label="Close contents"
              onClick={() => setDrawerOpen(false)}
              className="absolute inset-0 cursor-default border-0 bg-[rgba(20,20,15,.55)] p-0"
            />
            <aside
              className="relative flex h-full w-[min(324px,86vw)] min-h-0 flex-col bg-[#14140F]"
              style={RAIL_TOKENS}
            >
              <button
                onClick={() => setDrawerOpen(false)}
                aria-label="Close contents"
                className="absolute right-[10px] top-[10px] z-10 flex size-[36px] cursor-pointer items-center justify-center rounded-[8px] border border-border bg-transparent font-sans text-[15px] leading-none text-text-secondary"
              >
                ✕
              </button>
              {rail}
        ...
        {/* --------------------------------------------------------- Main */}
        <main className="flex min-w-0 flex-col bg-[var(--bg-sunken)]">
          <header
            className="sticky z-[40] border-b border-border bg-surface"
            style={{ top: NAV_H }}
          >
            <div className="flex flex-wrap items-center gap-[12px] px-[16px] py-[10px] min-[1020px]:flex-nowrap min-[1020px]:gap-[22px] min-[1020px]:px-[32px] min-[1020px]:py-[13px]">
              <button
                onClick={() => setDrawerOpen(true)}
                aria-label="Open contents"
                aria-expanded={drawerOpen}
                className="flex shrink-0 cursor-pointer items-center gap-[8px] rounded-[8px] border border-border bg-surface-raised px-[12px] py-[10px] font-sans text-[13px] font-semibold leading-none text-text-secondary min-[1020px]:hidden"
              >
                <span
                  aria-hidden="true"
                  className="flex w-[13px] shrink-0 flex-col gap-[3px]"
                >
                  <span className="h-[1.5px] rounded-px bg-current" />
                  <span className="h-[1.5px] rounded-px bg-current" />
                  <span className="h-[1.5px] rounded-px bg-current" />
                </span>
                Contents
              ...
              <div className="flex min-w-0 flex-[1_1_140px] flex-col gap-[4px] min-[1020px]:min-w-[220px] min-[1020px]:flex-[1_1_220px]">
                ...
                <div className="overflow-hidden text-ellipsis whitespace-nowrap font-sans text-[14px] font-semibold leading-[1.3] text-text min-[1020px]:text-[15px]">
                  {crumb.main}
                </div>
              </div>

              <div className="w-full min-[1020px]:w-[min(340px,32vw)] min-[1020px]:min-w-[150px] min-[1020px]:flex-[0_1_300px]">
                <Input
                  ref={searchRef}
                  placeholder="Search sections, clauses, illustrations"
                  value={q}
                  // 16px stops iOS Safari zooming the page on focus, which on a
                  // phone leaves the reader scrolled sideways.
                  className="text-[16px] min-[1020px]:text-[14px]"
                  onChange={(e) => {
                    const v = e.target.value;
                    setQ(v);
                    setModeOverride(
                      v.trim() ? "search" : idx >= 0 ? "read" : "toc",
                    );
                  }}
                />
              </div>
          ...
          <div className="px-[16px] pb-[56px] pt-[20px] min-[1020px]:px-[32px] min-[1020px]:pb-[96px] min-[1020px]:pt-[30px]">
            {/* --------------------------------------------------- Contents */}
            {mode === "toc" && (
              <div className="mx-auto flex max-w-[1000px] flex-col gap-[24px]">
                <div className="flex flex-col gap-[18px] rounded-[12px] border border-border bg-surface px-[20px] pb-[24px] pt-[26px] shadow-[0_1px_4px_rgba(0,0,0,.08)] min-[720px]:px-[38px] min-[720px]:pb-[30px] min-[720px]:pt-[34px]">
                  <div className="flex flex-wrap items-center gap-[10px] font-sans text-[12px] font-semibold uppercase leading-none tracking-[0.14em] text-primary-text">
                  ...
                  <h1 className="m-0 font-display text-[clamp(25px,7vw,38px)] font-semibold leading-[1.15] tracking-[-0.028em] text-text [text-wrap:pretty]">
                    The Digital Personal Data Protection Act, 2023
                  </h1>
                  <p className="m-0 max-w-[74ch] font-sans text-[16px] font-normal leading-[1.7] text-text-secondary [text-wrap:pretty]">
                    An Act to provide for the processing of digital personal
                    data in a manner that recognises both the right of
                    individuals to protect their personal data and the need to
                    process such personal data for lawful purposes and for
                  ...
                  <div className="flex flex-col gap-[12px] pt-[6px] min-[560px]:flex-row min-[560px]:flex-wrap">
                    <Button
                      variant="primary"
                ...
                <div className="flex flex-col gap-[12px]">
                  ...
                  <div className="grid grid-cols-[repeat(auto-fill,minmax(min(302px,100%),1fr))] gap-[16px]">
                    {CHAPTERS.map((c) => {
                ...
                {trimmed.length >= 2 && results.length === 0 && (
                  <div className="flex flex-col items-center gap-[8px] rounded-[12px] border border-dashed border-border bg-surface px-[24px] py-[56px] text-center">
                    ...
                    <div className="max-w-[44ch] font-sans text-[13.5px] font-normal leading-[1.6] text-text-muted">
                      ...
                      <em>Data Fiduciary</em>, <em>breach</em>, <em>child</em>{" "}
                      or <em>penalty</em>.
                    </div>
                  </div>
                )}
            ...
            {mode === "read" && page && (
              <div
                ref={readingPaneRef}
                className="mx-auto flex max-w-[840px] flex-col gap-[20px]"
              >
                <article className="overflow-hidden rounded-[12px] border border-border bg-surface shadow-[0_1px_4px_rgba(0,0,0,.08)]">
                  <div className="flex flex-wrap items-start gap-[14px] border-b border-border px-[18px] pb-[18px] pt-[20px] min-[720px]:flex-nowrap min-[720px]:gap-[18px] min-[720px]:px-[34px] min-[720px]:pb-[22px] min-[720px]:pt-[26px]">
                    <div className="flex size-[50px] shrink-0 flex-col items-center justify-center gap-[2px] rounded-[12px] border border-[rgba(180,50,26,.22)] bg-[rgba(180,50,26,.08)] min-[720px]:size-[58px]">
                      ...
                      <span className="font-mono text-[19px] font-bold leading-none tracking-[-0.02em] text-primary-text tabular-nums min-[720px]:text-[21px]">
                        {page.kind === "sch" ? "Sch" : page.n}
                      </span>
                    </div>
                    <div className="flex min-w-[180px] flex-1 flex-col gap-[8px]">
                      <div className="font-sans text-[12px] font-semibold uppercase leading-none tracking-[0.13em] text-text-muted">
                        {page.kind === "sch"
                      ...
                      {matchN > 0 && (
                        <div className="self-start rounded-full bg-[rgba(180,50,26,.12)] px-[10px] py-[4px] font-sans text-[12px] font-semibold leading-[1.4] text-primary-text">
                          {matchN} {matchN === 1 ? "match" : "matches"} for “
                          {hl}” highlighted below
                        </div>
                      )}
                    </div>
                    <button
                      onClick={toggleRead}
                      className={cn(
                        "flex shrink-0 cursor-pointer items-center gap-[8px] rounded-[8px] border px-[14px] py-[11px] font-sans text-[13px] font-semibold leading-none hover:border-primary-text min-[720px]:py-[9px] min-[720px]:text-[12.5px]",
                        isRead
                  ...
                  <div className="px-[18px] pb-[24px] pt-[20px] min-[720px]:px-[34px] min-[720px]:pb-[30px] min-[720px]:pt-[26px]">
                    {page.blocks.map((b, k) => {
                      if (b[0] === "p") {
                        return (
                          <p
                    ...
                    {page.kind === "sch" && (
                      <div className="flex flex-col gap-[16px]">
                        ...
                        {/* Below 720px the three columns stack: a 200px penalty
                            column beside prose has nowhere to go on a phone. */}
                        <div className="overflow-hidden rounded-[10px] border border-border">
                          ...
                          {SCHEDULE.rows.map((r) => (
                            <div
                              key={r.sl}
                              className="flex flex-col gap-[6px] border-b border-surface-raised px-[14px] py-[14px] min-[720px]:grid min-[720px]:grid-cols-[64px_minmax(0,1fr)_200px] min-[720px]:gap-0 min-[720px]:p-0"
                            >
                              <div className="font-mono text-[13px] font-semibold leading-[1.6] text-text-muted min-[720px]:px-[14px] min-[720px]:py-[15px]">
                                {r.sl}
                              </div>
                              <div className="font-sans text-[13.5px] font-normal leading-[1.7] text-text-secondary min-[720px]:px-[14px] min-[720px]:py-[15px]">
                                {r.breach}
                              </div>
                              <div className="font-sans text-[13px] font-semibold leading-[1.6] text-primary-text min-[720px]:px-[14px] min-[720px]:py-[15px]">
                                {r.penalty}
                              </div>
                            </div>
                          ))}
                ...
                <div className="grid grid-cols-1 gap-[14px] min-[720px]:grid-cols-2">
                  ...
                  {next && (
                    <button
                      onClick={() => goTo(idx + 1)}
                      className="flex w-full cursor-pointer flex-col items-start gap-[5px] rounded-[12px] border border-border bg-surface px-[18px] py-[16px] text-left hover:border-primary-text hover:bg-[rgba(180,50,26,.03)] min-[720px]:col-start-2 min-[720px]:items-end min-[720px]:text-right"
                    >
                ...
                <div className="flex items-center justify-center gap-[10px] pt-[2px]">
                  <span className="text-center font-sans text-[12px] font-semibold leading-[1.4] text-text-muted">
                    Section {idx + 1} of {TOTAL}
                    <span className="hidden min-[1020px]:inline">
                      {" "}
                      · use ← → to move through the Act
                    </span>
                  </span>
                </div>
              </div>
